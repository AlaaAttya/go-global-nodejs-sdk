 ...
 */
const transformRoomsPayload = (roomsArray) => {
    const roomsCount = roomsArray.length;
    const rooms = [];
    for (let i = 0; i < roomsCount; i++) {
        rooms.push({
            kidsCount: get(roomsArray, `${i}.kids`, 0),
            adultsCount: get(roomsArray, `${i}.adults`, 0),
        });
    }
...
const prepareJsonData = (searchParams, apiCredential) => (
    {
        Root: {
            ...
            Main: {
                ...
                Nights: 4,
                Rooms: {
                    Room: transformRoomsPayload(get(searchParams, 'rooms', [])),
                },
                Hotels: {
                    HotelId: [],
                },
